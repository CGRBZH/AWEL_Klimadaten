...
## Comparing metadata with OGD data
...
# COMPLETING MISSING DATA IN METADATA ---------------------
...
# MERGING METEO SCHWEIZ DATA TO DAILY VALUES -----------------------------------
...
saveRDS(tageswerte_komplett, "./Stadtklima_v2/tageswerte_komplett.RDS")

# Checking File size
print(object.size(tageswerte_komplett)/1e6)

print(file.info("./output/tageswerte_komplett.RDS")$size/1e6)
...
plot <-ggplot(data = tageswerte_komplett %>% filter(sensor == 531 & date >= max(date)-365), aes(x = as.Date(date))) + 
